...
//! Ported from codescratch `src/index/resolve.ts` (precedence + labels).
//!
//! Precedence per call site:
//!   1. import-binding  (strong) — callee is a named import we can follow to a file
//!   2. same-file       (strong) — one non-method symbol of that name in the same file
//!   3. receiver-unknown(weak)   — `recv.x`, no type info; unique repo-wide name if any
//!   4. unique-global   (weak)   — exactly one symbol repo-wide with that name
//!   5. unresolved      (weak)   — kept open, never faked
//!
//! Module resolution (WP-2A): relative → tsconfig `paths`/`baseUrl`(+`extends`)
//! → workspace packages. `export *` barrels followed at export lookup (depth ≤6).
...
/// Module-resolution config. Empty = relative + `index` only (v0.1 behavior).
